        ...
        <v-card class="white pt-8 my-3" height="88vh">
          <v-container>
            <v-card-title
              ...
              style="text-align: center; color: white; font-size: 18px; position: absolute; top: -25px; left: 0px; width: 100%; min-width: 100px; border-radius: 3px;"
              class="primary body-2"
            >
              Search Your Channels</v-card-title
            >
            <v-card-title
              v-else
              style="color: white; font-size: 18px; position: absolute; top: -25px; left: 25px; width: 30%; min-width: 200px; border-radius: 3px;"
              class="primary body-2"
            >
              Search Your Channels</v-card-title
            >
...
export default {
  ...
  methods: {
    ...
    async getCompany(id) {
      await this.$http
        .get('https://www.sowerkbackend.com/api/companies/' + id)
        .then(async (response) => {
          console.log(response.data, 'HEYYYYY')
          this.company = response.data
          for (let i = 0; i < response.data.locations.length; i++) {
            if (
              this.approvedVendorsList[i].approvedVendors[0] ===
              ...
              let company = {
                ...
                channelManagerFullName:
                  response.data.locations[i].contact_first_name +
                  ' ' +
                  response.data.locations[i].contact_last_name,
                channelManagerFirstName:
                  response.data.locations[i].contact_first_name,
                channelManagerLastName:
                  response.data.locations[i].contact_last_name,
