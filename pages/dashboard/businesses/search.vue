  ...
  <v-container class="px-0" style="max-width: 100%">
      ...
      <v-card class="my-4" style="width: 99%; height: auto; background-image: url('/tools-texture.png'); background-size: cover; background-position: bottom;" v-if="loadingInit">
        <v-row style="width:100%;" class="d-flex justify-center nowrap">
          ...
          <v-col cols="5" class="d-flex flex-column justify-center">
            ...
            <v-btn @click="businessSearchModalOpen" class="my-2 py-6 mx-auto my-auto" style="width: 80%" rounded color="primary">Business Search</v-btn>
            <v-btn @click="applicationSearchModalOpen" class="my-2 py-6 mx-auto my-auto" style="width: 80%" rounded color="primary">Application Search</v-btn>
          </v-col>
        </v-row>
      </v-card>
    </transition>
    ...
    <v-row style="width: 95%;" class="mx-auto">
      <v-col cols="12" class="d-flex flex-column justify-space-between">
        <transition name="slide-fade">
            <v-card class="white" color="mt-8" v-if="businessSearchRequestsModal && !applicationSearchRequestsModal">
        ...
        <transition name="slide-fade">
          <v-card class="white" color="mt-8" v-if="!businessSearchRequestsModal && applicationSearchRequestsModal">
            <v-card-title style="position: absolute; top: -30px; left: 25px; width: 40%; border-radius: 3px; font-size: 18px;" class="primary white--text font-weight-regular red-gradient" v-if="allApplications.length > 0 && !loading">All Applications - {{allApplications.length}}</v-card-title>
            <v-card-title style="position: absolute; top: -30px; left: 25px; width: 40%; border-radius: 3px; font-size: 18px;" class="primary white--text font-weight-regular red-gradient" v-else-if="allApplications.length === 0 && !loading">All Applications - 0</v-card-title>
            <v-text-field v-if="!loading" clearable outlined class="pt-12" style="width: 80%; margin-left: 10%;" label="Search By Application Name, Channel Name or Address, and Company Name" v-model="searchApplication" light></v-text-field>
            <v-data-table
...
export default {
  ...
  data() {
    return {
      ...
      applicationHeaders: [
        ...
        { text: 'Address', value: 'channelAddress', class: 'primary--text font-weight-bold text-h6 text-center', sortable: false },
        { text: 'Actions', value: 'actions', sortable: false, class: 'primary--text font-weight-bold text-h6 text-left' },
      ],
      allCompaniesHeaders: [
      ...
      allCompanies: [],
      allApplications: [],
      businessSearchRequestsModal: true,
      applicationSearchRequestsModal: false,
    }
  },
  computed: {
    currentUser() {
  ...
  methods: {
    ...
    async businessSearchModalOpen() {
      this.businessSearchRequestsModal = true
      this.applicationSearchRequestsModal = false
      await this.getAllCompanies();
    },
    async applicationSearchModalOpen() {
      this.businessSearchRequestsModal = false
      this.applicationSearchRequestsModal = true
      await this.getAllCompanies();
    },
    async getCompany() {
              ...
              .then(response => {
                ...
                response.data.location.forEach(location => {
                  if(location.userforms[0] === 'There are no userforms') {
                    ...
                    location.userforms = location.userforms.filter(userform => {
                      if(userform.applicationStatus === 1 || userform.applicationStatus === 2) {
                        let applicationListObj = {
                          ...userform,
                          channelName: location.name,
                          channelAddress: `${location.address} ${location.city}, ${location.state} ${location.zipcode}`,
                          companyName: this.allCompanies[i].account_name,
                          imageUrl: this.allCompanies[i].imgUrl,
                        }
                        this.allApplications.push(applicationListObj)
                        return userform
                      }
                    })
                  }
