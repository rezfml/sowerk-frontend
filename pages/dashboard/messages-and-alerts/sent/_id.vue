...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getCompany() {
        await this.$http.get('http://node-express-env.eba-vhau3tcw.us-east-2.elasticbeanstalk.com/api/companies/name/' + this.individualMessage.company)
          .then(async (response) => {
      ...
      async getLocations() {
        await this.$http.get('http://node-express-env.eba-vhau3tcw.us-east-2.elasticbeanstalk.com/api/locations/byCompaniesId/' + this.company.id)
          .then(response => {
