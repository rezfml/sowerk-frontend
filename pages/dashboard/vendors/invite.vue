...
<script>
  ...
  export default {
    ...
    methods: {
      async getCompany() {
        let {data, status} = await this.$http.get('http://www.sowerkbackend.com/api/companies/' + this.$store.state.user.user.user.companies_id).catch(e => e);
        this.company = data;
      ...
      async getBusinesses() {
        this.loading = true;
        this.locations = [];
        let {data, status} = await this.$http.get('http://www.sowerkbackend.com/api/companies/type/1').catch(e => e);
      ...
      async inviteProviders() {
        ...
        await this.$http.post('http://www.sowerkbackend.com/email', providersObject)
          .then(response => {
