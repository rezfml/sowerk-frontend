    ...
    <transition name="slide-fade">
      <v-row v-if="openUploadModelLoad" class="mt-8">
        <v-card style="background-color: white; height: 50vh; width: 100%;">
          ...
          <v-card-text class="pt-16 ml-4">Upload any company document or template that you will use to share with insurances to download, complete, and upload to SOWerk. Common items include master service agreements, independent contractor agreements, nondisclosure agreements, and tax examples.</v-card-text>
    ...
    methods: {
      ...
      async getInsuranceDocuments() {
        await this.$http.get('https://www.sowerkbackend.com/api/insurance/byCompanyId/' + this.currentUser.companies_id)
          .then(response => {
            console.log(response.data, 'response for insurance docs')
            this.insuranceDocuments = response.data
          })
          .catch(err => {
