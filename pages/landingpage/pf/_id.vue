<template>
  <div class="pfaccountlocationcontainer">
    <div class="pfaccountlocation" v-if="loading">
      ...
      <h1>{{propertymanagerVal.company_name}}</h1>
      <img />
    ...
    <div class="pfaccountsecondcolumn">
      <div>
        <p v-if="this.propertymanagerVal.locations">{{propertymanagerVal.locations.length}} Retail Locations</p>
        <div v-for='location in propertymanagerVal.locations' class="pfaccountlocationflexrow2">
          <p>Address: {{ location.address }}, {{ location.city }}, {{ location.state }}, {{ location.zipcode }}</p>
          <a :href="'/landingpage/pf/' + propertymanagerVal.id + '/location/' + location.id">></a>
        </div>
      </div>
